...
df_idxstats = pd.read_table(snakemake.input.idxstats).set_index('genome', drop=True)

depth_columns = 'genome position coverage'.split()
df_depth = pd.read_table(snakemake.input.depth, 
                         names=depth_columns)\
             .set_index('genome', drop=True)
df_depth_summary = df_depth.copy()
df_depth_summary['is_mapped'] = df_depth_summary['coverage'] > 0
genome_grouped = df_depth_summary.drop(columns=['position', ]).groupby(by='genome')
...
position_grouped = df_depth.groupby('position')
